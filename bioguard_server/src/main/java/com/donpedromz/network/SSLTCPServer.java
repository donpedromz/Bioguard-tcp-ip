...
public class SSLTCPServer implements INetworkService{
     ...
     * @param processingPolicy Política de procesamiento que se aplicará a los mensajes recibidos de los clientes.
     *                         No debe ser null.
     */
    public SSLTCPServer(ISSLConfig config, ProcessingPolicy processingPolicy) {
        this.config = config;
    ...
    public void start(){
        try(ServerSocket serverSocket = createSSLFactory().createServerSocket(config.getPort())){
            ...
            while(true){
                ...
                Thread clientThread = new Thread(new ClientHandler(clientSocket, processingPolicy));
                clientThread.start();
            }
        } catch (Exception e) {
